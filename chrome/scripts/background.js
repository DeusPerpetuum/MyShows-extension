...
function checkToken() {
	chrome.storage.local.get(["token", "refresh_token", "token_expires_in"]).then((result) => {
		if (result.token == "" || !result.token || result.token == "error") return chrome.storage.local.set({ token: "error" });

		if (!result.refresh_token) return chrome.storage.local.set({ token: "error" });
...
function getProfileInfo() {
	...
	request({
		...
		.catch((error) => {
			chrome.storage.local.set({ token: "error" });
			console.error(error);
		});
}
...
function checkEpisodeRequest(episodeID) {
	return request({
		...
		if (result.error) {
			if (result.error.code == 401) chrome.storage.local.set({ token: "error" });
			return false;
		}
